class LocalitiesController < ApplicationController
  ...
  def relacion
    ...
    @localidadnombrefecha = Event.where("nombre = ?  AND localities_id = ?", params[:nombre],params[:localities_id])
    respond_to do |format|
        format.json { render 'relacion', status: :created, location: @events }
    end
  end
  ...
  def newlocality
     @locality = Locality.new(tipo: params[:tipo], precio: params[:precio], fecha_limite: params[:fecha_limite])
          @locality.save
    redirect_to '/localities/'+@locality.id.to_s+'.json'
  end
  ...
  private
    ...
    def locality_params
      params.require(:locality).permit(:tipo, :precio, :fecha_limite)
    end
end
